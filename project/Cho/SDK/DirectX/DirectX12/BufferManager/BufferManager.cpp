...
{
	ConstantBuffer buffer(desc);
	buffer.CreateResource(m_Device, desc.sizeInBytes);
	// コンテナに移動
...
{
	StructuredBuffer buffer(desc);
	buffer.CreateResource(m_Device, static_cast<size_t>(desc.numElements*desc.structuredByteStride));
	// SRVの設定
	D3D12_SHADER_RESOURCE_VIEW_DESC srvDesc{};
	srvDesc.Format = DXGI_FORMAT_UNKNOWN;
	srvDesc.Shader4ComponentMapping = D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING;
