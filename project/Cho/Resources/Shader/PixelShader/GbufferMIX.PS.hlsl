...
PixelShaderOutput main(VertexShaderOutput input)
{
    ...
    for (int dirLIndex = 0; dirLIndex < MAX_DIRECTIONAL_LIGHTS; dirLIndex++) {
        ...
        // 拡散反射
        float3 diffuseDirectionalLight = gLight.directionalLights[dirLIndex].color.rgb * cos * gLight.directionalLights[dirLIndex].intensity;
        
        // 鏡面反射
        float3 specularDirectionalLight = gLight.directionalLights[dirLIndex].color.rgb * gLight.directionalLights[dirLIndex].intensity * specularPow * float3(1.0f, 1.0f, 1.0f);
    ...
    for (int pointLIndex = 0; pointLIndex < MAX_POINT_LIGHTS; pointLIndex++) {
        ...
        // half lambert
        float NdotLPointLight = dot(normal, pointLightDirection);
        float cosPointLight = pow(NdotLPointLight * 0.5f + 0.5f, 2.0f);
        
        // 拡散反射
        float3 diffusePointLight = gLight.pointLights[pointLIndex].color.rgb * cosPointLight * gLight.pointLights[pointLIndex].intensity * factor;
    ...
    for (int spotLIndex = 0; spotLIndex < MAX_SPOT_LIGHTS; spotLIndex++) {
        ...
        // half lambert
        float NdotLSpotLight = dot(normal, -spotLightDirectionOnSurface);
        float cosSpotLight = pow(NdotLSpotLight * 0.5f + 0.5f, 2.0f);
        // 拡散反射
        float3 diffuseSpotLight = gLight.spotLights[spotLIndex].color.rgb * gLight.spotLights[spotLIndex].intensity * cosSpotLight * finalAttenuation;
